...
{
    memory.lecture(address, externalMemData);
    _delay_ms(6);
}
...
void Usart::transmitExternalMem()
{
    ...
    size1 = USARTReceive();
    instructionNumber = (uint16_t)size1;
    memory.ecriture(0x0000, size1);
    _delay_ms(6);
    instructionNumber = instructionNumber << 8;

    size2 = USARTReceive();
    memory.ecriture(0x0001, size2);
    _delay_ms(6);
    instructionNumber |= (uint16_t)size2;

    for (uint16_t i = 2; i < instructionNumber; i++)
    {
        memory.ecriture(0x0000 + i, USARTReceive());
        _delay_ms(6);
    }
}
...
void Usart::debugExternalMem()
{
    // This function allows us to see the complete content 
    // of the external memory
    // After the make install, we have to do serieViaUSB -l
    uint8_t *valeurLue=nullptr;
    uint16_t instructionNumbre = getInstructionNumber();
    for (uint16_t i = 0; i < instructionNumbre; i++)
    {
        readExtarnalMem(uint16_t(0x0000) + i, valeurLue);
        transmitDataInt(DataType::UNIQUE_INT, *valeurLue, {});
    }
}